...
Versioning continues the semantic patch series from the SignalFlow template (`1.0.7` → `1.0.8` …). The footer and Admin Changelog page read the current version from `package.json` (now **1.0.73**).
...
## [1.0.73] - 2026-09-15 08:20 IST

### Changed
- **Home** — landing page visual redesign aligned to the Stitch mockup while keeping the left sidebar navigation; refreshed hero, trust stats, how-it-works, testimonials, pricing, and Instagram grid styling
- **Home** — added Featured Courses and Explore Products sections (placeholder course and PMS lineup in `client-config` until the real catalog is wired)

## [1.0.72] - 2026-09-15 07:53 IST

### Changed
- **Products** — `/products` uses more horizontal space on large viewports (wider page container, slightly wider filter sidebar, and more gap between sidebar and product list)
...
### Changed
- **Products** — `/products` catalog redesigned with a verified-catalog hero and live stat tiles (product count, weighted average rating, free resources, Cashfree checkout), sidebar filters (category, price bucket, minimum rating), sort control, featured band, and expandable product rows with shared category icons, colors, and savings badges
- **Products** — category feature bullets, icons, and color tokens centralized in `src/lib/products.ts` so the catalog, product detail page, and home scroll section stay aligned

## [1.0.69] - 2026-09-14 22:15 IST

### Changed
- **Account → My Subscriptions** — rebuilt as a 3-column executive-terminal dashboard (Plan & Billing + billing history | PMS portfolio terminal | courses, indicators/e-books, and memberships), with shared `TerminalCard` / stat tiles / billing-period progress ring building blocks; collapses to a single column on smaller screens

## [1.0.68] - 2026-09-14 21:50 IST

### Changed
- **Products** — flagship A-to-Z course (`a-to-z-stock-market-course`) updated to webinar mentorship copy and ₹25,000 pricing; stray `a-to-z-masterclass-mentorship` row deactivated (not deleted) when re-running `seed-products.ts`
- **Products** — Golden Line & Speedometer Volatility Suite placeholder price set to ₹2,500 with no strikethrough in seed data
...
### Changed
- Dashboard **Trade Stats** — Avg %/Trade, Best Trade, and Worst Trade now show as one range chart on a shared 0% baseline (colored by win/loss) instead of three separate stat cards; Total Signals stays its own stat tile

## [1.0.65] - 2026-09-13 22:20 IST

### Added
- **Account → Settings** — subscribers can change their login password; Google-only accounts (no password yet) see **Set a Password** instead
- **Admin → Settings** — verify current admin password and generate a new `ADMIN_PASSWORD_HASH` value to paste into Vercel (env-based admin login is unchanged until you redeploy)

### Changed
- Shared `MIN_PASSWORD_LENGTH` (6 characters) in `src/lib/password.ts` for registration and all password-change flows
...
### Changed
- Admin **Products** catalog moved to its own sidebar item (`/admin/products`); **News & Alerts** is alerts-only again

## [1.0.62] - 2026-09-13 19:25 IST

### Added
- Admin **News & Alerts** — **Products** tab to manage the full `/products` catalog (add, edit, active toggle, delete with purchase guard) alongside news alerts

### Changed
- **Products** catalog rows — expanded detail shows a larger thumbnail beside the long description and feature bullets
...
### Changed
- **Insert Sample Signal** — matches the current Manual Signal Entry form: adds a `NOW` line for CMP, pre-fills manual entry (including example Setup Type and Comments), and expands the parsed preview to show expiry and sell price
- Admin Help — parse/manual-entry docs and live sample text updated for the grouped form layout

## [1.0.59] - 2026-09-12 18:55 IST

### Added
- Setup Type options: Jadu Ki Chaddi, Golden Line, Black Box, Jackpot, Lakshman Rekha
- Manual Signal Entry — optional **Comments** field beside Setup Type (saved as signal admin note and posted to Updates)

## [1.0.58] - 2026-09-12 18:32 IST

### Added
- Signal **Setup Type** (ORB, Breakout, Breakdown, Reversal, Continuation, Trend Following, Range Bound, Scalp, News Based, Other) — optional on existing signals; defaults to Other on manual entry; shown in Trade Log, admin Manage Signals, and as a badge on Ongoing Trade cards

### Changed
- Ongoing Trade panel — each open signal is one full-width stacked card with risk/reward and that trade’s Updates thread together (no side-by-side mini-cards or separate Updates block below)

## [1.0.57] - 2026-09-11 16:32 IST
...
## [1.0.55] - 2026-09-11 08:25 IST

### Changed
- Site and admin navigation moved from a top navbar to a left sidebar; mobile uses a hamburger drawer

## [1.0.54] - 2026-09-11 07:32 IST

### Added
- Home page Products panel — horizontally scrolling catalog strip with links to each product page, toggled from Admin → Settings (`productsScrollEnabled`, off by default)
...
### Changed
- Products catalog rows — "View Details" is now a full-width outline button stacked below the buy button instead of an underlined text link

## [1.0.51] - 2026-09-11 06:11 IST

### Changed
- Portfolio Management Service product copy updated with real plan details from the client's marketing graphic — group pool structure, mentorship inclusions, risk cover, minimum return, and profit-share terms

## [1.0.50] - 2026-09-03 09:05 IST

### Added
- Admin Settings page (Profile menu → Settings) — broker connect master switch + Dhan/Goodwill picker, weekly digest email toggle, News & Market Alerts toggle, all live with no redeploy
- Subscriber Settings page (Profile menu → Settings) — weekly digest email opt-in/out and notification bell alerts on/off
...
### Added
- "Need New Demat A/C" option in the Current Trading Broker dropdown (registration and profile)
- Profile page WhatsApp CTA to open a new Demat account under our partner broker referral when no broker is on record or the new option is selected

## [1.0.46] - 2026-08-31 23:21 IST

### Added
- Points column (sell price − entry price) before P&L % in the Trade Log, admin Manage Signals table, and dashboard Recent Signals table

## [1.0.45] - 2026-08-31 23:05 IST

### Changed
- Home page hero social icons (WhatsApp, Instagram, Telegram, YouTube) now use brand light teal (`#23a3d1`) instead of gold

## [1.0.44] - 2026-08-31 23:03 IST
...
## [1.0.43] - 2026-08-31 18:34 IST

### Changed
- Home page trust stats updated: 1,500+ active students, 25+ years of market experience, 250+ live sessions hosted, 3,000+ total beneficiaries
