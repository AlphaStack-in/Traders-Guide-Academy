...
function instrumentPrefix(
  instrument: InstrumentValue | null | undefined,
  stockSymbol?: string | null,
) {
  const label = formatInstrumentLabel(instrument, stockSymbol);
  return label ? `${label} ` : "";
}
...
export function formatNewSignalMessage(signal: {
  ...
  return [
    `🟡 <b>New Signal — ${instrumentPrefix(signal.instrument, signal.stockSymbol)}${signal.strike} ${signal.optionType}</b>`,
...
export function formatSignalUpdateMessage(signal: {
  ...
  return [
    `${emoji} <b>Trade Update — ${instrumentPrefix(signal.instrument, signal.stockSymbol)}${signal.strike} ${signal.optionType}</b>`,
