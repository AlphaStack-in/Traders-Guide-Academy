...
import { GitCommit, Info, CheckCircle2, Server } from "lucide-react";
...
export function BuildVersionIndicator({ className = "", buildInfo: initialInfo }: BuildVersionIndicatorProps) {
  ...
  return (
    <div className={`relative inline-flex items-center text-xs text-muted-foreground/60 ${className}`}>
      <button
        type="button"
        onClick={() => setOpen(!open)}
        className="group inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border border-white/5 bg-white/[0.02] hover:bg-white/10 hover:text-foreground transition-all duration-200 cursor-pointer outline-none"
        title="Click for build & release metadata"
      >
        <GitCommit className="h-3 w-3 text-primary/70 group-hover:text-primary transition-colors" />
        <span className="font-mono text-[11px] font-medium tracking-tight">
          v{info.version} · build {info.gitSha}
        </span>
      </button>
      ...
      {open && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4 sm:absolute sm:inset-auto sm:bottom-full sm:left-0 sm:mb-2 sm:p-0"
          onClick={() => setOpen(false)}
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="w-72 rounded-2xl border border-white/10 bg-[#0d0e14]/95 p-4 shadow-2xl backdrop-blur-md flex flex-col gap-3 font-sans text-xs text-foreground"
          >
            <div className="flex items-center justify-between border-b border-white/10 pb-2">
              <span className="font-bold tracking-wide flex items-center gap-1.5 text-foreground">
                <Server className="h-3.5 w-3.5 text-primary" />
                SignalFlow Deployment
              </span>
            ...
            <div className="pt-2 border-t border-white/10 flex items-center justify-between text-[10px] text-muted-foreground">
              <span>Authoritative source revision</span>
              <button
                type="button"
                onClick={() => setOpen(false)}
